"""
swarm_vectorized.py - Optimized SwarmManager with vectorized operations
...
class SwarmManagerVectorized:
    ...
    def update(self, dt=None):
        ...
        self.auction_tasks_vectorized(comm_mask)
        
        # Calculate steering forces
        task_steer = self.calculate_task_steer()
        formation_steer = self.calculate_formation_steer_vectorized()
        ...
        # Vectorized Boundary Handling - OPTIMIZED (no Python loop!)
        self.resolve_boundary_vectorized(dt)
        
        # Hard Collision Resolution
        self.resolve_collisions()

    def resolve_boundary_vectorized(self, dt):
        """
        OPTIMIZED: Vectorized boundary handling with safety checks
        Replaces the sequential for-loop in original code
        """
        ...
        if self.env.boundary == "wrap":
            ...
            # Apply bounce (reverse velocity and clamp position)
        ...
        # Apply boundary repulsion (vectorized with epsilon safety)
    ...
    def calculate_formation_steer_vectorized(self):
        """
        OPTIMIZED: Vectorized formation control
        Original had a sequential for-loop (lines 263-274)
        ...
        # Vectorized row and side calculation (replaces for-loop)
        rows = np.arange(self.num_boids) // 2
        sides = np.where(np.arange(self.num_boids) % 2 == 0, 1, -1)
        
        # Special case for first drone (leader)
        rows[0] = 0
        sides[0] = 0
    ...
    def auction_tasks_vectorized(self, comm_mask):
        """
        OPTIMIZED: Vectorized auction with conflict resolution
        Original had sequential loops (lines 205-217)
        """
        # Generate bids for unassigned tasks
        ...
        # Find conflicts: drones that can see each other and have same task
        task_matrix = self.assigned_tasks[:, np.newaxis] == self.assigned_tasks
        same_task = task_matrix & (self.assigned_tasks != -1)[:, np.newaxis]
        conflicts = comm_mask & same_task & ~np.eye(self.num_boids, dtype=bool)
        
        # For each conflict, the drone with higher bid (or higher ID if tie) loses
        for i in range(self.num_boids):
            if self.assigned_tasks[i] == -1:
                continue
            
            # Find drones that conflict with i and have higher priority
            conflict_indices = np.where(conflicts[i])[0]
            if len(conflict_indices) == 0:
                continue
            
            # Check if any conflicting drone has better bid
            better_bids = (self.bids[conflict_indices] < self.bids[i]) | (
                (self.bids[conflict_indices] == self.bids[i]) & 
                (self.ids[conflict_indices] < self.ids[i])
            )
            
            if np.any(better_bids):
    ...
    def steer(self, vectors, subtract_velocity=False):
        """Same as original steer method with safety checks"""
        magnitudes = np.linalg.norm(vectors, axis=1)
        valid = magnitudes > 1e-6  # Use small epsilon instead of 0

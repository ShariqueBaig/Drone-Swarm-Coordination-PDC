...
class Environment:
    ...
    def get_obstacle_arrays(self):
        ...
        pos = np.array([[o[0], o[1]] for o in obs], dtype=float)
        rad = np.array([o[2] for o in obs], dtype=float)
        return pos, rad

    # ── Waypoint (set by simulation UI) ───────────────────────────────────────
    target_waypoint = None   # None or np.array([x, y])

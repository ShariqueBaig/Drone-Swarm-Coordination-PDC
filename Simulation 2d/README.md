...
- _Visual Indicator_: Drones flash a **bright yellow ring** the exact moment they receive a new task from a neighbor.
...
- **Stored drone orientation:** Added an explicit per-drone `orientations` array (angle in radians) in `swarm.py`. Orientations are initialized from velocity and updated each physics tick; the renderer in `simulation.py` falls back to this orientation when a drone is nearly stationary.
- **Tests & run status:** Unit tests (`test_optimizations.py`) pass locally and the simulation has been exercised interactively during development. To run tests:
  ```bash

...
class Flock:
    ...
    def update(self):
        ...
        # Boundary handling (hard wall or wrapping)
        boundary_steer = np.zeros((self.num_boids, 2))
        if not config.wrap_around:
            # Hard-wall repulsion near margin
            left_dist = self.positions[:, 0]
            right_dist = config.width - self.positions[:, 0]
            bottom_dist = self.positions[:, 1]
            top_dist = config.height - self.positions[:, 1]

            for i in range(self.num_boids):
                if left_dist[i] < config.boundary_margin:
                    boundary_steer[i, 0] += config.boundary_repulsion_strength / max((left_dist[i] + 1e-4) ** 2, 1e-4)
                if right_dist[i] < config.boundary_margin:
                    boundary_steer[i, 0] -= config.boundary_repulsion_strength / max((right_dist[i] + 1e-4) ** 2, 1e-4)
                if bottom_dist[i] < config.boundary_margin:
                    boundary_steer[i, 1] += config.boundary_repulsion_strength / max((bottom_dist[i] + 1e-4) ** 2, 1e-4)
                if top_dist[i] < config.boundary_margin:
                    boundary_steer[i, 1] -= config.boundary_repulsion_strength / max((top_dist[i] + 1e-4) ** 2, 1e-4)
            boundary_steer = self.steer(boundary_steer, 0)
        ...
        if config.wrap_around:
            ...
            self.positions[:, 0] = np.clip(self.positions[:, 0], 0, config.width)
            self.positions[:, 1] = np.clip(self.positions[:, 1], 0, config.height)
